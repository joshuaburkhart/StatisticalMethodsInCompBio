...
```{r}
cv_result = matrix() # need a matrix for storing our cv loop result
#Columns for cv_result: [error rate, feature indices, feature means, features sds, svm, markov blanket]
...
for(cv_idx in seq(1:10)){
  ...
  design = model.matrix(hw3B.x.classes_train)
  fit <- lmFit(hw3B.x.features_train_normalized,hw3B.x.classes_train)
  fit <- eBayes(fit)
  topTable(fit,coef = 2)
  ...
  x.ilen <-
  intersect(limmaRes %>% rownames(),hw3B.x.net_input %>% colnames()) %>% length()
  ...
  hw3B.xq.svm_input <-
  data.frame(t(hw3B.xq.features_train_normalized_w))
  ...
  xq.class_ag <-
  table(predict(xq.model,hw3B.xq.svm_input),hw3B.xq.classes_train) %>%
  classAgreement()
