...
for(cv_idx in seq(1:3)){
  ...
  if(x.err_rate < x.cv_result[5]){
    x.cv_result[[1]] <- NA #x.train_sd
    x.cv_result[[2]] <- NA #x.train_mean
    x.cv_result[3] <- x.ilen / nrow(hw3B.x.net_input)
    x.cv_result[[4]] <- x.mhc
    x.cv_result[5] <- x.err_rate
    x.cv_result[[6]] <- x.model
  }
  
  if(xq.err_rate < xq.cv_result[5]){
    xq.cv_result[[1]] <- xq.train_sd
    xq.cv_result[[2]] <- xq.train_mean
    xq.cv_result[3] <- NA
    xq.cv_result[[4]] <- xq.mhc
    xq.cv_result[5] <- xq.err_rate
    xq.cv_result[[6]] <- xq.model
  }
  
  print(c(cv_idx,x.cv_result))
  print(c(cv_idx,xq.cv_result))
}
...
```{r}
    ...
    # recover results from cross validation

    x.cv_lm_pct <- x.cv_result[3]
    x.cv_mhc <- x.cv_result[[4]]
    x.cv_err_rate <- x.cv_result[5]
    x.cv_model <- x.cv_result[[6]]
    
    xq.cv_train_sd <- xq.cv_result[[1]]
    xq.cv_train_mean <- xq.cv_result[[2]]
    xq.cv_mhc <- xq.cv_result[[4]]
    xq.cv_err_rate <- xq.cv_result[5]
    xq.cv_model <- xq.cv_result[[6]]
    ...
    for (i in 1:nrow(hw3B.x.features_test)){
      ...
      for (j in 1:ncol(hw3B.x.features_test)){
        ...
        hw3B.x.features_test_normalized[i,j] =
          (hw3B.x.features_test[i,j] - x.cv_train_mean[i]) / x.cv_train_sd[i]
      }
    }
  }
  ...
  for (i in 1:nrow(hw3B.xq.features_test)){
    ...
    for (j in 1:ncol(hw3B.xq.features_test)){
      ...
      hw3B.xq.features_test_normalized[i,j] =
        (hw3B.xq.features_test[i,j] - xq.cv_train_mean[i]) / xq.cv_train_sd[i]
    }
  }
  ...
  #test
  x.class_ag <-
    table(predict(x.cv_model,hw3B.x.svm_input),hw3B.x.classes_test) %>%
    classAgreement()
  
  xq.class_ag <-
    table(predict(xq.cv_model,hw3B.xq.svm_input),hw3B.xq.classes_test) %>%
    classAgreement()
