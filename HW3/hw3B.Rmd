...
#now we can access our selected columns with
#> hw3A.xq.features_train_normalized[xq.train_w_idx,]
```
...
library(bnlearn)
library(Rgraphviz)
# Unfortunately, we'll have to sloppily transpose our df for this
bn.hc <- hc(data.frame(t(hw3A.xq.features_train_normalized[xq.train_w_idx,])))
bn.gs <- gs(data.frame(t(hw3A.xq.features_train_normalized[xq.train_w_idx,])))
bn.fast.iamb <- fast.iamb(data.frame(t(hw3A.xq.features_train_normalized[xq.train_w_idx,])))
par(mfrow = c(1,3))
graphviz.plot(bn.hc)
graphviz.plot(bn.gs)
...
> work through [3] 5.3. Network analysis and manipulation
